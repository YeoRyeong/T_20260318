...
void UFSM::Process(int CurrenState, std::string CurrentCondition)
{
	for (int i = 0; i < 5; ++i)
	{
		if (Transitions[i].CurrentState == CurrenState &&
			Transitions[i].Condition == CurrentCondition)
		{
			std::cout << Transitions[i].NextState << std::endl;

			return;
		}
	}

	std::cout << GetStateName(CurrenState) << std::endl;
}
